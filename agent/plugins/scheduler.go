...
func PluginRun(plugin *Plugin) {
	...
	if !file.IsExist(fpath) {
		log.Info("no such plugin:", fpath)
		return
	}

	debug := g.Config().Debug
	if debug {
		log.Info(fpath, "running...")
	}
	...
	if errStr != "" {
		...
		if _, err = file.WriteString(logFile, errStr); err != nil {
			log.Infof("[ERROR] write log to %s fail, error: %s\n", logFile, err)
		}
	}
	...
	if isTimeout {
		...
		if err != nil {
			log.Info("[ERROR] kill process", fpath, "occur error:", err)
		}
	...
	if err != nil {
		log.Info("[ERROR] exec plugin", fpath, "fail. error:", err)
		return
	}
	...
	if len(data) == 0 {
		if debug {
			log.Info("[DEBUG] stdout of", fpath, "is blank")
		}
		return
	}
	...
	if err != nil {
		log.Infof("[ERROR] json.Unmarshal stdout of %s fail. error:%s stdout: \n%s\n", fpath, err, stdout.String())
		return
	}
